...
def extract_all_genres(movies: pd.DataFrame):
    genres = set()
...
def movie_has_selected_genre(genre_string: str, selected_genres) -> bool:
    if not selected_genres:
        return True
    movie_genres = set(str(genre_string).split("|"))
    return bool(movie_genres.intersection(set(selected_genres)))
...
def recommend_movies(profiles: pd.DataFrame, selected_movie_ids, aspect_weights, selected_genres=None, top_n=10):
    aspect_cols = ASPECTS
    ...
    # Optional genre filter selected by the new user.
    if selected_genres:
        candidates = candidates[candidates["genres"].apply(lambda g: movie_has_selected_genre(g, selected_genres))].copy()
...
def run_small_evaluation(profiles, ratings, sample_users=75, k=10):
    ...
    for user_id in users:
        ...
        train = liked[:5]
        test = set(liked[5:])
        recs = recommend_movies(profiles, train, {a: 0 for a in ASPECTS}, selected_genres=None, top_n=k)
...
st.subheader("2. Genre filter")
st.write("Optionally choose one or more preferred genres. Recommendations will be filtered to movies matching at least one selected genre.")
available_genres = extract_all_genres(movies_df)
...
st.subheader("3. Aspect preferences")
cols = st.columns(3)
aspect_weights = {}
...
if st.button("Generate recommendations", type="primary"):
    results = recommend_movies(profiles_df, selected_movie_ids, aspect_weights, selected_genres=selected_genres, top_n=top_n)
    st.subheader("4. Recommended movies")
    if results.empty:
        st.warning("No recommendations matched the selected genre filter. Try selecting fewer genres or clearing the genre filter.")
...
if st.button("Run quick evaluation"):
    ...
    if evaluation:
        ...
        st.success("Offline evaluation completed using anonymised MovieLens test records.")
    else:
        st.warning("Not enough eligible users for evaluation.")

with st.expander("How this prototype works"):
    st.write("""
    This prototype uses the real MovieLens latest-small dataset. Because MovieLens does not include full movie review paragraphs, the artefact uses structured ratings, genres and user-generated free-text tags as available MovieLens evidence. Movie aspect profiles are created from genres, tags and rating quality. A new user profile is created from selected liked movies, optional genre preferences and explicit aspect preferences. Recommendations are generated by filtering candidate movies by genre where selected, comparing the new user profile to movie profiles, and ranking suitable movies.
    """)
